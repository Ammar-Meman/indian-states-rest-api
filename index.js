...
app.get("/states/highest-gdp",(req,res)=>{
  const sortedStates = [...states].sort((a,b)=>b.gdp-a.gdp);
  const highestGDPState = sortedStates[0];

  res.json(highestGDPState);
})
...
app.put("/states/:id/budget",(req,res)=>{
  const id = parseInt(req.params.id);
  const annualBudget = req.body.annualBudget;
...
app.delete("/states/name/:stateName",(req,res)=>{
  ...
  const index = states.findIndex(s => s.name.toLowerCase() === stateName);
